...
namespace HemisphericalAmbient {
    public class SdkMesh {
        ...
        private readonly List<SdkMeshVertexBufferHeader> _vertexBuffers = new List<SdkMeshVertexBufferHeader>();
        private readonly List<SdkMeshIndexBufferHeader> _indexBuffers = new List<SdkMeshIndexBufferHeader>();
        private readonly List<SdkMeshMesh> _meshes = new List<SdkMeshMesh>();
        private readonly List<SdkMeshSubset> _subsets = new List<SdkMeshSubset>();
        private readonly List<SdkMeshFrame> _frames = new List<SdkMeshFrame>();
        private readonly List<SdkMeshMaterial> _materials = new List<SdkMeshMaterial>();
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct SdkMeshVertexBufferHeader {
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct SdkMeshIndexBufferHeader {
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct SdkMeshMaterial {
            ...
            public SdkMeshMaterial(BinaryReader reader) {
                ...
                MaterialInstancePath = Encoding.Default.GetString(reader.ReadBytes(MaxMaterialPath));
                DiffuseTexture = Encoding.Default.GetString(reader.ReadBytes(MaxTextureName));
                NormalTexture = Encoding.Default.GetString(reader.ReadBytes(MaxTextureName));
                SpecularTexture = Encoding.Default.GetString(reader.ReadBytes(MaxTextureName));
