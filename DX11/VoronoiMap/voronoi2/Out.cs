...
namespace VoronoiMap.voronoi2 {
    static class Out {
        public static void Bisector(Edge edge) {
            Console.WriteLine("line({0} {1}x+{2}y={3}, bisecting {4} {5}\n", edge.EdgeID, edge.A, edge.B, edge.C, edge.Reg[LR.Left].SiteID, edge.Reg[LR.Right].SiteID);
        }

        public static void Endpoint(Edge e) {
            Console.WriteLine("e {0} {1} {2}\n", 
                e.EdgeID, 
                e.EP[LR.Left] != null ? e.EP[LR.Left].SiteID : -1,
                e.EP[LR.Right] != null ? e.EP[LR.Right].SiteID : -1
                );
        }
